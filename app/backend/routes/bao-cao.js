...
module.exports = function baoCaoRoutes({ pool, authenticate, authorize }) {
  ...
  function validateBaoCao(body) {
    ...
    if (body.nguoi_gui_sdt && !/^\d{9,11}$/.test(body.nguoi_gui_sdt))
      {return 'Số điện thoại người gửi phải 9-11 chữ số';}
    if (body.nguoi_gui_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.nguoi_gui_email))
  ...
  router.post(
    ...
    async (req, res, next) => {
      ...
      if (!(req.files || []).every(hasSafeImageMagic)) {
        removeUploadedFiles(req.files);
        return res.status(400).json({ error: 'Tệp ảnh không hợp lệ theo chữ ký nội dung' });
      }
      ...
      if (req.body.mo_ta && req.body.mo_ta.length > 10000) {
        removeUploadedFiles(req.files);
      ...
      try {
        ...
        await audit(client, req, AUDIT_ACTIONS.REPORT_CREATE, 'bao_cao_vi_pham', r.rows[0].id);
  ...
  router.get(
    '/api/v1/bao-cao',
    authenticate,
    authorize('report.view_own'),
    async (req, res, next) => {
      try {
        const all = req.user.permissions.includes('case.view');
        const r = await pool.query(
          `SELECT bc.id,bc.ma_bao_cao,bc.nguoi_gui_ten,bc.mo_ta,bc.dia_chi,bc.created_at,${pointSelect('bc')},(SELECT count(*)::int FROM tep_dinh_kem t WHERE t.entity_type='bao_cao' AND t.entity_id=bc.id) AS anh_count FROM bao_cao_vi_pham bc ${all ? '' : 'WHERE bc.nguoi_gui_id=$1'} ORDER BY bc.created_at DESC`,
          all ? [] : [req.user.id]
        );
  ...
  router.post(
    ...
    async (req, res, next) => {
      ...
      try {
        await client.query('BEGIN');
        const bc = (
          await client.query('SELECT * FROM bao_cao_vi_pham WHERE id=$1', [req.params.id])
        ).rows[0];
        if (!bc) {
          await client.query('ROLLBACK');
        ...
        const existing = (
          await client.query('SELECT id,ma_ho_so FROM ho_so WHERE bao_cao_id=$1 LIMIT 1', [bc.id])
        ).rows[0];
        if (existing) {
          await client.query('ROLLBACK');
          return res
            .status(409)
            .json({ error: 'Báo cáo đã được chuyển thành hồ sơ', data: existing });
        }
        ...
        const attachments = (
          await client.query(
            "SELECT ten_goc, duong_dan, loai_file, kich_thuoc FROM tep_dinh_kem WHERE entity_type='bao_cao' AND entity_id=$1",
            [bc.id]
          )
        ).rows;
        for (const att of attachments) {
          await client.query(
            "INSERT INTO tep_dinh_kem (entity_type, entity_id, ten_goc, duong_dan, loai_file, kich_thuoc, nguoi_tai_id) VALUES ('ho_so', $1, $2, $3, $4, $5, $6)",
            [hoSo.id, att.ten_goc, att.duong_dan, att.loai_file, att.kich_thuoc, req.user.id]
          );
        }
        await audit(client, req, AUDIT_ACTIONS.CASE_CREATE, 'ho_so', hoSo.id, {
          from_bao_cao: bc.id,
  ...
  router.get(
    ...
    async (req, res, next) => {
      try {
        ...
        const anh = (
          await pool.query(
            "SELECT id, ten_goc, duong_dan, loai_file, kich_thuoc, created_at FROM tep_dinh_kem WHERE entity_type='bao_cao' AND entity_id=$1 ORDER BY created_at",
            [bc.id]
          )
        ).rows;
        const hoSo = (
          await pool.query(
            'SELECT id, ma_ho_so, trang_thai FROM ho_so WHERE bao_cao_id=$1 LIMIT 1',
            [bc.id]
          )
        ).rows[0];
