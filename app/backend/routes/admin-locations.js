...
module.exports = function adminLocationsRoutes({ pool, authenticate, authorize }) {
  ...
  router.get('/api/v1/admin/quan-huyen', authenticate, authorize('admin.locations'), async (_req, res, next) => {
  ...
  router.post('/api/v1/admin/quan-huyen', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
  ...
  router.patch('/api/v1/admin/quan-huyen/:id', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const { ma, ten, boundary } = req.body || {};
  ...
  router.delete('/api/v1/admin/quan-huyen/:id', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  router.get('/api/v1/admin/phuong-xa', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    try {
  ...
  router.post('/api/v1/admin/phuong-xa', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
  ...
  router.patch('/api/v1/admin/phuong-xa/:id', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const { ma, ten, quan_huyen_id, boundary } = req.body || {};
    if (ma !== undefined && !MA_RE.test(ma)) return res.status(400).json({ error: 'Mã phải từ 1-20 ký tự, chỉ gồm chữ, số và dấu gạch ngang' });
    if (ten !== undefined && (!ten?.trim() || ten.trim().length > TEN_MAX)) return res.status(400).json({ error: `Tên không được rỗng và tối đa ${TEN_MAX} ký tự` });
  ...
  router.delete('/api/v1/admin/phuong-xa/:id', authenticate, authorize('admin.locations'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
