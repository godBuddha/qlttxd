...
module.exports = function adminCatalogsRoutes({ pool, authenticate, authorize }) {
  ...
  router.get('/api/v1/admin/loai-vi-pham', authenticate, authorize('admin.users'), async (_req, res, next) => {
  ...
  router.post('/api/v1/admin/loai-vi-pham', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
  ...
  router.patch('/api/v1/admin/loai-vi-pham/:id', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const { code, ten, mo_ta, so_thu_tu } = req.body || {};
  ...
  router.delete('/api/v1/admin/loai-vi-pham/:id', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  router.get('/api/v1/admin/hanh-vi', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    try {
  ...
  router.post('/api/v1/admin/hanh-vi', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
  ...
  router.patch('/api/v1/admin/hanh-vi/:id', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const { loai_vi_pham_id, dieu, khoan, diem, ten, mo_ta, is_active } = req.body || {};
  ...
  router.delete('/api/v1/admin/hanh-vi/:id', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  router.get('/api/v1/admin/muc-phat', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    try {
  ...
  router.post('/api/v1/admin/muc-phat', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
  ...
  router.patch('/api/v1/admin/muc-phat/:id', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const { hanh_vi_id, nhom_cong_trinh, muc_toi_thieu, muc_toi_da } = req.body || {};
  ...
  router.delete('/api/v1/admin/muc-phat/:id', authenticate, authorize('admin.users'), async (req, res, next) => {
    if (!requirePool(pool, res)) return;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
