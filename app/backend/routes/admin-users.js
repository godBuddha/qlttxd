  ...
  router.patch(
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        try {
          await client.query('BEGIN');
          const existing = await client.query('SELECT id, is_active FROM users WHERE id=$1', [
            userId,
          ]);
          if (!existing.rows[0]) {
            await client.query('ROLLBACK');
          ...
          if (isDeactivating || isRemovingAdmin) {
            ...
            if (adminCount.rows[0].cnt < 1) {
              ...
              if (currentAdmin.rows[0].is_admin) {
                ...
                return res
                  .status(400)
                  .json({ error: 'Không thể khóa/xóa quyền quản trị viên cuối cùng' });
              }
            }
          }
          ...
          if (req.user.id === userId && isRemovingAdmin) {
            ...
            if (selfAdminCount.rows[0].cnt <= 1) {
              ...
              return res
                .status(400)
                .json({ error: 'Không thể tự gỡ quyền quản trị khi chỉ còn 1 admin' });
            }
          }
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(roleId))
          {return res.status(400).json({ error: 'ID vai trò không hợp lệ' });}
        ...
        try {
          ...
          if (permission_ids.length) {
            const validPerms = await client.query('SELECT id FROM permissions WHERE id = ANY($1)', [
              permission_ids,
            ]);
            if (validPerms.rows.length !== permission_ids.length) {
              await client.query('ROLLBACK');
              return res.status(400).json({ error: 'Một hoặc nhiều quyền không tồn tại' });
            }
          }
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        const r = await pool.query(
         ...
         ${where} ORDER BY al.thoi_gian DESC LIMIT $${vals.length - 1} OFFSET $${vals.length}`,
          vals
        );
