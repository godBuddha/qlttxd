...
const { requirePool, coordinate, audit, nextCode, pointSelect, createThongBao, fetchHoSoDocxData, vnFont } = require('../utils/helpers');
...
module.exports = function hoSoRoutes({ pool, authenticate, authorize }) {
  ...
  function validateHoSo(b) {
    ...
    if (b.loai_vi_pham_id && !UUID_RE.test(b.loai_vi_pham_id)) return 'loai_vi_pham_id không hợp lệ (cần UUID)';
  ...
  router.post('/api/v1/ho-so', authenticate, authorize('case.update'), async (req, res, next) => {
    ...
    try { await client.query('BEGIN'); let offender = null; if (b.nguoi_vi_pham?.ten && b.nguoi_vi_pham?.loai_chu_the) offender = (await client.query('INSERT INTO nguoi_vi_pham (loai_chu_the,ten,cmnd_cccd,dia_chi,sdt,email,nguoi_dai_dien) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id',[b.nguoi_vi_pham.loai_chu_the,b.nguoi_vi_pham.ten,b.nguoi_vi_pham.cmnd_cccd||null,b.nguoi_vi_pham.dia_chi||null,b.nguoi_vi_pham.sdt||null,b.nguoi_vi_pham.email||null,b.nguoi_vi_pham.nguoi_dai_dien||null])).rows[0].id;
      ...
      await audit(client, req, AUDIT_ACTIONS.CASE_CREATE, 'ho_so', r.rows[0].id); await client.query('COMMIT'); res.status(201).json({ data: r.rows[0] });
    } catch(e) { await client.query('ROLLBACK'); next(e); } finally { client.release(); }
  });

  router.get('/api/v1/ho-so', authenticate, authorize('case.view'), async (req, res, next) => { try { const vals=[]; const add=(sql,v)=>{vals.push(v);return `${sql}$${vals.length}`}; const w=['h.deleted_at IS NULL']; if(req.query.trang_thai)w.push(add('h.trang_thai=',req.query.trang_thai)); if(req.query.quan_huyen_id)w.push(add('h.quan_huyen_id=',req.query.quan_huyen_id)); if(req.query.tu_ngay)w.push(add('h.created_at>=',req.query.tu_ngay)); if(req.query.den_ngay)w.push(add('h.created_at<=',req.query.den_ngay)); if(req.query.q)w.push(add('(h.ma_ho_so ILIKE ',`%${req.query.q}%`) + ` OR h.mo_ta ILIKE $${vals.length} OR h.dia_chi ILIKE $${vals.length} OR lvp.ten ILIKE $${vals.length})`); const limit=Math.min(Math.max(Number(req.query.limit)||20,1),100),page=Math.max(Number(req.query.page)||1,1); vals.push(limit,(page-1)*limit); const countResult=await pool.query(`SELECT count(*)::int FROM ho_so h LEFT JOIN loai_vi_pham lvp ON lvp.id=h.loai_vi_pham_id WHERE ${w.join(' AND ')}`,vals.slice(0,-2)); const r=await pool.query(`SELECT h.id,h.ma_ho_so,h.trang_thai,h.dia_chi,h.created_at,h.updated_at,${pointSelect('h')} FROM ho_so h LEFT JOIN loai_vi_pham lvp ON lvp.id=h.loai_vi_pham_id WHERE ${w.join(' AND ')} ORDER BY h.created_at DESC LIMIT $${vals.length-1} OFFSET $${vals.length}`,vals); res.json({data:r.rows,page,limit,total:countResult.rows[0].total,totalPages:Math.ceil(countResult.rows[0].total/limit)}); } catch(e){next(e);} });

  router.get('/api/v1/ho-so/:id', authenticate, authorize('case.view'), async (req,res,next)=>{try { const h=await pool.query(`SELECT h.*,${pointSelect('h')},row_to_json(bc) bao_cao,row_to_json(nvp) nguoi_vi_pham,row_to_json(uxl) nguoi_xu_ly FROM ho_so h LEFT JOIN bao_cao_vi_pham bc ON bc.id=h.bao_cao_id LEFT JOIN nguoi_vi_pham nvp ON nvp.id=h.nguoi_vi_pham_id LEFT JOIN (SELECT u.id,u.full_name,u.username FROM users u) uxl ON uxl.id=h.nguoi_xu_ly_id WHERE h.id=$1 AND h.deleted_at IS NULL`,[req.params.id]); if(!h.rows[0])return res.status(404).json({error:'Không tìm thấy hồ sơ'}); const [bb,qd,kp,anh]=await Promise.all([pool.query('SELECT * FROM bien_ban WHERE ho_so_id=$1 ORDER BY created_at',[req.params.id]),pool.query('SELECT * FROM quyet_dinh WHERE ho_so_id=$1 ORDER BY created_at',[req.params.id]),pool.query('SELECT * FROM khac_phuc WHERE ho_so_id=$1 ORDER BY created_at',[req.params.id]),h.rows[0].bao_cao_id ? pool.query('SELECT id,ten_goc,duong_dan,loai_file,kich_thuoc,created_at FROM tep_dinh_kem WHERE entity_type=$1 AND entity_id=$2 ORDER BY created_at',['bao_cao',h.rows[0].bao_cao_id]) : Promise.resolve({rows:[]})]); res.json({data:{...h.rows[0],bien_ban:bb.rows,quyet_dinh:qd.rows,khac_phuc:kp.rows,anh:anh.rows}});}catch(e){next(e);}});

  router.patch('/api/v1/ho-so/:id/trang-thai', authenticate, authorize('case.update'), async(req,res,next)=>{try {const nextState=req.body?.trang_thai;if(!STATES.has(nextState))return res.status(400).json({error:'Trạng thái hồ sơ không hợp lệ'});const old=(await pool.query('SELECT trang_thai FROM ho_so WHERE id=$1',[req.params.id])).rows[0];if(!old)return res.status(404).json({error:'Không tìm thấy hồ sơ'});if(!TRANSITIONS[old.trang_thai]?.includes(nextState))return res.status(400).json({error:'Chuyển trạng thái không hợp lệ'});const r=await pool.query('UPDATE ho_so SET trang_thai=$1 WHERE id=$2 RETURNING id,ma_ho_so,trang_thai,updated_at',[nextState,req.params.id]);await audit(pool,req,AUDIT_ACTIONS.CASE_STATUS_CHANGE,'ho_so',req.params.id,{from:old.trang_thai,to:nextState});try {const hs=await pool.query('SELECT nguoi_nop_id,nguoi_xu_ly_id FROM ho_so WHERE id=$1',[req.params.id]);const recipients=new Set([hs.rows[0]?.nguoi_nop_id,hs.rows[0]?.nguoi_xu_ly_id].filter(Boolean));for(const uid of recipients){if(uid!==req.user.id)await createThongBao(pool,{nguoi_nhan_id:uid,ho_so_id:req.params.id,loai:'trang_thai',tieu_de:`${r.rows[0].ma_ho_so}: chuyển trạng thái`,noi_dung:`Từ ${STATE_LABELS[old.trang_thai]||old.trang_thai} → ${STATE_LABELS[nextState]||nextState}`});;if(uid!==req.user.id&&isSmtpConfigured())await createThongBao(pool,{nguoi_nhan_id:uid,ho_so_id:req.params.id,loai:'trang_thai',tieu_de:`${r.rows[0].ma_ho_so}: chuyển trạng thái`,noi_dung:`Từ ${STATE_LABELS[old.trang_thai]||old.trang_thai} → ${STATE_LABELS[nextState]||nextState}`,kenh:'email',trang_thai:'cho_gui'});}}catch(_n){/*notification failure should not block status update*/}res.json({data:r.rows[0]});}catch(e){next(e);}});

  // PUT /api/v1/ho-so/:id/phan-cong — assign handler
  router.put('/api/v1/ho-so/:id/phan-cong', authenticate, authorize('case.assign'), async (req, res, next) => {
    ...
    try {
      ...
      await audit(pool, req, AUDIT_ACTIONS.CASE_ASSIGN, 'ho_so', req.params.id, { old_can_bo_id: oldAssigneeId || null, new_can_bo_id: can_bo_id, can_bo_ten: handler.rows[0].full_name });
  ...
  router.get('/api/v1/ho-so/:id/xuat-quyet-dinh.docx', authenticate, authorize('case.update'), async (req, res, next) => {
    try {
      const data = await fetchHoSoDocxData(pool, req.params.id);
      if (!data) return res.status(404).json({ error: 'Không tìm thấy hồ sơ' });
  ...
  router.get('/api/v1/ho-so/:id/xuat-bien-ban.pdf', authenticate, authorize('case.update'), async (req, res, next) => {
    try {
      const data = await fetchHoSoDocxData(pool, req.params.id);
      if (!data) return res.status(404).json({ error: 'Kh\u00f4ng t\u00ecm th\u1ea5y h\u1ed3 s\u01a1' });
      if (!data.bien_ban) return res.status(400).json({ error: 'H\u1ed3 s\u01a1 ch\u01b0a c\u00f3 bi\u00ean b\u1ea3n' });
      ...
      doc.moveDown(0.3);
      doc.fontSize(11).font('VN').text('S\u1ed1: ' + (data.bien_ban.ma_bien_ban || '\u2026'), { align: 'center' });
      doc.moveDown(1);
      const diaChiVP = [data.ho_so.dia_chi, data.phuong_xa && data.phuong_xa.ten, data.quan_huyen && data.quan_huyen.ten].filter(Boolean).join(', ') || '\u2026';
      doc.fontSize(10).font('VN');
      doc.text('\u0110\u1ecba ch\u1ec9 vi ph\u1ea1m: ' + diaChiVP);
      doc.text('Ng\u01b0\u1eddi vi ph\u1ea1m: ' + (data.nguoi_vi_pham && data.nguoi_vi_pham.ten || '\u2026'));
      doc.text('H\u00e0nh vi vi ph\u1ea1m: ' + (data.hanh_vi && data.hanh_vi.ten || '\u2026'));
      doc.text('M\u00f4 t\u1ea3: ' + (data.ho_so.mo_ta || data.bien_ban.noi_dung || '\u2026'));
      doc.text('M\u1ee9c ph\u1ea1t d\u1ef1 ki\u1ebfn: ' + (data.bien_ban.muc_phat_du_kien != null ? Number(data.bien_ban.muc_phat_du_kien).toLocaleString('vi-VN') + ' \u0111\u1ed3ng' : '\u2026'));
  ...
  router.get('/api/v1/ho-so/:id/xuat-quyet-dinh.pdf', authenticate, authorize('case.update'), async (req, res, next) => {
    try {
      const data = await fetchHoSoDocxData(pool, req.params.id);
      if (!data) return res.status(404).json({ error: 'Kh\u00f4ng t\u00ecm th\u1ea5y h\u1ed3 s\u01a1' });
      if (!data.quyet_dinh) return res.status(400).json({ error: 'H\u1ed3 s\u01a1 ch\u01b0a c\u00f3 quy\u1ebft \u0111\u1ecbnh' });
      ...
      doc.fontSize(11).font('VN').text('\u0110\u1ed9c l\u1eadp - T\u1ef1 do - H\u1ea1nh ph\u00fac', { align: 'center' });
      doc.moveDown(1);
      doc.fontSize(14).font('VN-Bold').text('CH\u1ee6 T\u1ecaCH \u1ee8Y BAN NH\u00c2N D\u00c2N ' + (data.quan_huyen && data.quan_huyen.ten ? data.quan_huyen.ten.toUpperCase() : '\u2026'), { align: 'center' });
      doc.moveDown(0.3);
      doc.fontSize(11).font('VN-Bold').text('S\u1ed1: ' + (data.quyet_dinh.ma_quyet_dinh || '\u2026'), { align: 'center' });
      doc.moveDown(0.5);
      ...
      doc.text('T\u00ean ng\u01b0\u1eddi vi ph\u1ea1m: ' + (data.nguoi_vi_pham && data.nguoi_vi_pham.ten || '\u2026'));
      doc.text('CMND/CCCD: ' + (data.nguoi_vi_pham && data.nguoi_vi_pham.cmnd_cccd || '\u2026'));
      doc.text('H\u00e0nh vi: ' + (data.hanh_vi && data.hanh_vi.ten || '\u2026'));
      const dieuKhoan = data.hanh_vi ? '\u0110i\u1ec1u ' + (data.hanh_vi.dieu || '16') + ', Kho\u1ea3n ' + (data.hanh_vi.khoan || '\u2026') : '\u2026';
      doc.text('\u0110i\u1ec1u/Kho\u1ea3n: ' + dieuKhoan + ' N\u0110 16/2022/N\u0110-CP');
      const diaChiVP = [data.ho_so.dia_chi, data.phuong_xa && data.phuong_xa.ten, data.quan_huyen && data.quan_huyen.ten].filter(Boolean).join(', ') || '\u2026';
      doc.text('\u0110\u1ecba ch\u1ec9 vi ph\u1ea1m: ' + diaChiVP);
      doc.moveDown(0.5);
      doc.fontSize(11).font('VN-Bold').text('H\u00ecnh th\u1ee9c x\u1eed ph\u1ea1t:');
      doc.fontSize(10).font('VN').text('Ph\u1ea1t ti\u1ec1n: ' + (data.quyet_dinh.so_tien_phat != null ? Number(data.quyet_dinh.so_tien_phat).toLocaleString('vi-VN') + ' \u0111\u1ed3ng' : '\u2026'));
