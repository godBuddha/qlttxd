    ...
    async (_req, res, next) => {
      try {
        const [status, district, month] = await Promise.all([
          pool.query(
            'SELECT trang_thai,count(*)::int AS so_luong FROM ho_so WHERE deleted_at IS NULL GROUP BY trang_thai ORDER BY trang_thai'
          ),
          pool.query(
            'SELECT q.id,q.ten,count(h.id)::int AS so_luong FROM quan_huyen q LEFT JOIN ho_so h ON h.quan_huyen_id=q.id AND h.deleted_at IS NULL GROUP BY q.id,q.ten ORDER BY q.ten'
          ),
          pool.query(
            "SELECT to_char(date_trunc('month',created_at),'YYYY-MM') thang,count(*)::int AS so_luong FROM ho_so WHERE deleted_at IS NULL GROUP BY 1 ORDER BY 1 DESC"
          ),
        ]);
        res.json({
          data: { theo_trang_thai: status.rows, theo_quan: district.rows, theo_thang: month.rows },
        });
      } catch (e) {
        next(e);
      }
      ...
      try {
        ...
        if (loai === 'csv') {
          ...
          doc
            .fontSize(16)
            .font('VN-Bold')
            .text('Báo cáo thống kê hồ sơ vi phạm', { align: 'center' });
          doc.moveDown(0.5);
          doc
            .fontSize(10)
            .font('VN')
          ...
          rows.forEach((row) => {
            ...
            vals.forEach((v, i) =>
              doc.text(String(v || ''), cols[i], doc.y, { continued: i < vals.length - 1 })
            );
            doc.moveDown(0.3);
          });
