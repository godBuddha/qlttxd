...
async function main() {
  ...
  const server = app.listen(PORT, '127.0.0.1', async () => {
    ...
    try {
      ...
      check(
        'CORS: Access-Control-Allow-Origin present (with CORS_ORIGIN set)',
        !!acao,
        `value=${acao || 'MISSING'}`
      );
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (i === 0)
          {check('Login wrong → 401 (not 429 yet)', r.status === 401, `status=${r.status}`);}
      }
      ...
      check(
        'Admin login still works after wrong attempts',
        login.status === 200,
        `status=${login.status}`
      );
