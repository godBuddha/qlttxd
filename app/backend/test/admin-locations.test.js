...
test.before(async () => {
  ...
  await pool.query(
    "DELETE FROM user_roles WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'leader.loc%' OR username LIKE 'citizen.loc%')"
  );
  await pool.query(
    "DELETE FROM users WHERE username LIKE 'leader.loc%' OR username LIKE 'citizen.loc%'"
  );

  // Create leader user for 403 tests
  const _leaderRes = await json('/api/v1/admin/users', {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'X-Auth-Token': adminToken },
    body: JSON.stringify({
      username: 'leader.loc',
      password: 'Leader@2026',
      full_name: 'Lãnh đạo test',
      email: 'leader.loc@test.local',
      phone: '0902000001',
      roles: ['leader'],
    }),
  });
  ...
  await json('/api/v1/admin/users', {
    ...
    headers: { 'content-type': 'application/json', 'X-Auth-Token': adminToken },
    body: JSON.stringify({
      username: 'citizen.loc',
      password: 'Citizen@2026',
      full_name: 'Công dân test',
      email: 'citizen.loc@test.local',
      phone: '0903000001',
      roles: ['citizen'],
    }),
  });
  ...
  await pool.query(
    "DELETE FROM audit_log WHERE bang_bi_tac_dong IN ('quan_huyen','phuong_xa') AND chi_tiet->>'ma' LIKE 'TEST-%'"
  );
...
test('POST tạo phường mới → 201', async () => {
  const result = await json('/api/v1/admin/phuong-xa', {
    ...
    body: JSON.stringify({
      ma: 'TEST-0101',
      ten: 'Phường Test 0101',
      quan_huyen_id: createdQuanHuyenId,
      boundary: VALID_MULTI,
    }),
  });
  assert.equal(result.response.status, 201);
...
test('POST phường quan_huyen_id không tồn tại → 404', async () => {
  const result = await json('/api/v1/admin/phuong-xa', {
    ...
    body: JSON.stringify({
      ma: 'TEST-0103',
      ten: 'Phường quận không tồn tại',
      quan_huyen_id: '00000000-0000-0000-0000-000000000000',
    }),
  });
  assert.equal(result.response.status, 404);
});
...
test('audit_log có bản ghi thay đổi địa điểm', async () => {
  ...
  assert.ok(
    ...
    `Expected at least 4 audit records, got ${result.rows.length}`
  );
