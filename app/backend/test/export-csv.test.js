...
test.before(async () => {
  ...
  await pool.query(
    "DELETE FROM role_permissions WHERE role_id IN (SELECT id FROM roles WHERE code LIKE 'test_%')"
  );
  ...
  await createUser(
    {
      username: 'leader.csv',
      password: 'Test@2026',
      full_name: 'Lãnh đạo CSV',
      email: 'leader@csv.local',
      roles: ['leader'],
    },
    adminToken
  );
  await createUser(
    {
      username: 'citizen.csv',
      password: 'Test@2026',
      full_name: 'Công dân CSV',
      email: 'citizen@csv.local',
      roles: ['citizen'],
    },
    adminToken
  );
  ...
  await createUser(
    {
      username: 'stats.csv',
      password: 'Test@2026',
      full_name: 'Thống kê CSV',
      email: 'stats@csv.local',
      roles: ['test_stats_only'],
    },
    adminToken
  );
...
test('CSV export: admin nhận CSV với header đúng và có dữ liệu', async () => {
  ...
  assert.ok(
    lines.length >= 2,
    `CSV should have header + at least 1 data row, got ${lines.length} lines`
  );
...
test('CSV export: filter theo quan_huyen_id không crash', async () => {
  const { response } = await raw(
    '/api/v1/thong-ke/xuat?loai=csv&quan_huyen_id=00000000-0000-0000-0000-000000000000',
    {
      headers: { authorization: `Bearer ${adminToken}` },
    }
  );
...
test('CSV export: filter theo ngày không crash', async () => {
  const { response } = await raw(
    '/api/v1/thong-ke/xuat?loai=csv&tu_ngay=2025-01-01&den_ngay=2025-12-31',
    {
      headers: { authorization: `Bearer ${adminToken}` },
    }
  );
