...
function secret() {
  ...
  if (!s) {
    ...
    console.warn(
      '[helpers] JWT_SECRET not set — using random fallback (tokens will not survive restart)'
    );
...
function requirePool(pool, res) {
  if (!pool) {
    res.status(503).json({ error: 'Cơ sở dữ liệu chưa sẵn sàng' });
    return false;
  }
  return true;
}
...
async function audit(pool, req, action, table, id, detail = {}) {
  await pool.query(
    'INSERT INTO audit_log (nguoi_dung_id, hanh_dong, bang_bi_tac_dong, id_ban_ghi, chi_tiet, ip, request_id) VALUES ($1,$2,$3,$4,$5,$6,$7)',
    [
      req.user?.id || null,
      action,
      table,
      id || null,
      JSON.stringify(sanitizeObject(detail)),
      req.ip,
      req.requestId || null,
    ]
  );
...
async function nextCode(pool, prefix) {
  ...
  const result = await pool.query('SELECT next_business_code($1, $2::regclass) AS code', [
    prefix,
    sequence,
  ]);
...
function pointSelect(alias = '') {
  const p = alias ? `${alias}.` : '';
  return `CASE WHEN ${p}toa_do IS NULL THEN NULL ELSE json_build_object('lat', ST_Y(${p}toa_do), 'lng', ST_X(${p}toa_do)) END AS toa_do`;
}
...
async function createThongBao(
  clientOrPool,
  { nguoi_nhan_id, ho_so_id, loai, tieu_de, noi_dung, kenh, trang_thai }
) {
...
async function fetchHoSoDocxData(pool, hoSoId) {
  ...
  const [nvpRes, hvRes, lvpRes, bbRes, qdRes] = await Promise.all([
    hs.nguoi_vi_pham_id
      ? pool.query('SELECT * FROM nguoi_vi_pham WHERE id=$1', [hs.nguoi_vi_pham_id])
      : Promise.resolve({ rows: [null] }),
    hs.hanh_vi_id
      ? pool.query('SELECT * FROM hanh_vi_vi_pham WHERE id=$1', [hs.hanh_vi_id])
      : Promise.resolve({ rows: [null] }),
    hs.loai_vi_pham_id
      ? pool.query('SELECT * FROM loai_vi_pham WHERE id=$1', [hs.loai_vi_pham_id])
      : Promise.resolve({ rows: [null] }),
    pool.query('SELECT * FROM bien_ban WHERE ho_so_id=$1 ORDER BY created_at DESC LIMIT 1', [
      hoSoId,
    ]),
    pool.query('SELECT * FROM quyet_dinh WHERE ho_so_id=$1 ORDER BY created_at DESC LIMIT 1', [
      hoSoId,
    ]),
  ]);
  return {
    ho_so: hs,
    nguoi_vi_pham: nvpRes.rows[0],
    hanh_vi: hvRes.rows[0],
    loai_vi_pham: lvpRes.rows[0],
    bien_ban: bbRes.rows[0],
    quyet_dinh: qdRes.rows[0],
