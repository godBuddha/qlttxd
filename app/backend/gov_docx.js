...
function signatureBlock(nguoiKy) {
  return [
    ...
    new Paragraph({
      ...
      spacing: { after: 60 },
      children: [new TextRun({ text: 'Người có thẩm quyền', bold: true, size: 24, font: 'Times New Roman', italics: true })],
    }),
    new Paragraph({
      alignment: AlignmentType.RIGHT,
      spacing: { after: 200 },
      children: [new TextRun({ text: '(Ký, đóng dấu)', size: 22, font: 'Times New Roman', italics: true })],
    }),
    emptyLine(),
    emptyLine(),
    new Paragraph({
      alignment: AlignmentType.RIGHT,
      children: [new TextRun({ text: nguoiKy || '……………………', bold: true, size: 24, font: 'Times New Roman' })],
    }),
  ];
}
...
async function generateBienBan(data) {
  const { ho_so, bien_ban, nguoi_vi_pham, hanh_vi, loai_vi_pham, quan_huyen, phuong_xa, nguoi_lap } = data;
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        bodyText(`Vào hồi …… giờ …… phút, ${formatDate(bien_ban.thoi_gian_lap || bien_ban.created_at)}, tại ${ho_so.dia_chi || '………'}${phuong_xa ? `, ${phuong_xa.ten}` : ''}${quan_huyen ? `, ${quan_huyen.ten}` : ''}, chúng tôi gồm:`, true),
        ...
        labelValue('   Loại chủ thể: ', nguoi_vi_pham?.loai_chu_the === 'ca_nhan' ? 'Cá nhân' : nguoi_vi_pham?.loai_chu_the === 'to_chuc' ? 'Tổ chức' : '………'),
        ...
        labelValue('Điều/Khoản: ', hanh_vi ? `Điều ${hanh_vi.dieu || '16'}, Khoản ${hanh_vi.khoan || '…'}${hanh_vi.diem ? `, Điểm ${hanh_vi.diem}` : ''}` : '………'),
        ...
        // Mức phạt dự kiến
        labelValue('Mức phạt dự kiến: ', formatMoney(bien_ban.muc_phat_du_kien || ho_so.muc_phat_du_kien)),
        labelValue('Hình thức xử lý: ', bien_ban.hinh_thuc_xu_phat === 'phat_tien' ? 'Phạt tiền' : bien_ban.hinh_thuc_xu_phat || 'Phạt tiền'),
        emptyLine(),
        ...
        bodyText('Biên bản này được lập để ghi nhận hành vi vi phạm hành chính trong lĩnh vực hoạt động xây dựng theo quy định tại Nghị định 16/2022/NĐ-CP.', true),
        emptyLine(),
        bodyText('Biên bản được lập thành 02 bản, 01 bản giao cho người vi phạm, 01 bản lưu tại cơ quan.', true),
...
async function generateQuyetDinh(data) {
  const { ho_so, quyet_dinh, bien_ban, nguoi_vi_pham, hanh_vi, loai_vi_pham, quan_huyen, phuong_xa, nguoi_ky } = data;
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          ...
          spacing: { after: 120 },
          children: [
            new TextRun({ text: `Số: ${quyet_dinh.ma_quyet_dinh || '………'}`, bold: true, size: 24, font: 'Times New Roman' }),
          ],
        }),
        emptyLine(),
        ...
        bodyText(`Căn cứ Nghị định 16/2022/NĐ-CP ngày 28 tháng 01 năm 2022 của Chính phủ quy định xử phạt vi phạm hành chính về xây dựng;`, true),
        bodyText(`Căn cứ Nghị định 30/2020/NĐ-CP ngày 05 tháng 3 năm 2020 của Chính phủ về tổ chức và hoạt động của Thanh tra xây dựng;`, true),
        quyet_dinh.can_cu_phap_ly ? bodyText(`Căn cứ ${quyet_dinh.can_cu_phap_ly};`, true) : emptyLine(),
        bodyText(`Xét biên bản vi phạm hành chính số ${bien_ban?.ma_bien_ban || '………'} ${formatDate(bien_ban?.thoi_gian_lap || bien_ban?.created_at)};`, true),
        emptyLine(),
        ...
        bodyText(`Điều ${hanh_vi?.dieu || '16'}, Khoản ${hanh_vi?.khoan || '…'}${hanh_vi?.diem ? `, Điểm ${hanh_vi.diem}` : ''} Nghị định 16/2022/NĐ-CP ngày 28 tháng 01 năm 2022 của Chính phủ quy định xử phạt vi phạm hành chính về xây dựng.`, true),
        emptyLine(),
        ...
        emptyLine(),
        bodyText('Quyết định này có hiệu lực kể từ ngày ký. Người vi phạm phải chấp hành quyết định trong thời hạn 10 ngày kể từ ngày nhận quyết định.', true),
        bodyText('Trường hợp người vi phạm không tự nguyện chấp hành thì bị cưỡng chế thi hành theo quy định của pháp luật.', true),
        emptyLine(),

        // Điều 3: Khiếu nại
        centeredBold('Điều 3. Khiếu nại, khởi kiện', 24),
        emptyLine(),
        bodyText('Người bị xử phạt có quyền khiếu nại đến Chủ tịch Ủy ban nhân dân hoặc khởi kiện tại Tòa án theo quy định của pháp luật.', true),
        emptyLine(),
