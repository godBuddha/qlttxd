...
export function AdminLocationsPage({ api, notify }) {
  ...
  const filteredWards = selectedDistrict
    ? wards.filter((w) => String(w.quan_huyen_id) === String(selectedDistrict.id))
    : wards;
  let previewPolygons = [];
  if (form.boundary.trim()) {
    try {
      ...
      if (parsed.type === 'Feature') previewPolygons = [parsed];
        ...
        previewPolygons = [{ type: 'Feature', geometry: parsed, properties: {} }];
    } catch {
  ...
  const change = (e) => {
    ...
    if (e.target.name === 'boundary') setBoundaryError('');
  };
  const openCreateDistrict = () => {
    setModalType('district');
    setEditItem(null);
    setForm({ ma: '', ten: '', quan_huyen_id: '', boundary: '' });
    setBoundaryError('');
    setShowModal(true);
  };
  const openCreateWard = () => {
    ...
    setForm({
      ma: '',
      ten: '',
      quan_huyen_id: selectedDistrict?.id ? String(selectedDistrict.id) : '',
      boundary: '',
    });
    setBoundaryError('');
    setShowModal(true);
  ...
  const openEdit = (item, type) => {
    ...
    setForm({
      ma: item.ma,
      ten: item.ten,
      quan_huyen_id: item.quan_huyen_id ? String(item.quan_huyen_id) : '',
      boundary: item.boundary
        ? typeof item.boundary === 'string'
          ? item.boundary
          : JSON.stringify(item.boundary, null, 2)
        : '',
    });
    setBoundaryError('');
    setShowModal(true);
  ...
  async function save(e) {
    ...
    if (!/^[0-9A-Za-z-]{1,20}$/.test(form.ma.trim()))
      return notify('Mã phải từ 1-20 ký tự, chỉ gồm chữ, số và dấu gạch ngang.', 'error');
    if (modalType === 'ward' && !form.quan_huyen_id)
      return notify('Phải chọn quận/huyện cho phường/xã.', 'error');
    let boundaryPayload = null;
    if (form.boundary.trim()) {
      try {
        const parsed = JSON.parse(form.boundary);
        if (!parsed.type) return setBoundaryError('GeoJSON phải có trường type.');
        boundaryPayload = parsed;
      } catch {
        return setBoundaryError('GeoJSON không hợp lệ. Kiểm tra lại định dạng JSON.');
      }
    }
    ...
    try {
      ...
      if (editItem) {
        ...
        notify(`Đã cập nhật ${modalType === 'district' ? 'quận/huyện' : 'phường/xã'}.`, 'success');
      } else {
        await api(ep, { method: 'POST', body: JSON.stringify(payload) });
        notify(`Đã tạo ${modalType === 'district' ? 'quận/huyện' : 'phường/xã'} mới.`, 'success');
      }
      setShowModal(false);
      load();
    } catch (err) {
  ...
  async function handleDelete(item, type) {
    ...
    try {
      ...
      await api(`${ep}/${item.id}`, { method: 'DELETE' });
      notify(`Đã xóa ${label}.`, 'success');
    ...
    <>
      <div className="page-title">
        <div>
          ...
          <p>Quản lý quận/huyện và phường/xã — đơn vị hành chính trong hệ thống.</p>
        </div>
      </div>
      <div className="two-col">
        ...
        <section className="panel">
          ...
          >
            <h3 style={{ margin: 0 }}>
              {selectedDistrict ? `Phường/Xã — ${selectedDistrict.ten}` : 'Phường/Xã'}
            </h3>
                ...
                {filteredWards.length ? (
                  filteredWards.map((w) => (
                    <tr key={w.id}>
                      <td>{w.ten}</td>
                      <td>{w.ma}</td>
                      <td>{w.quan_huyen_ten || '—'}</td>
                      <td style={{ whiteSpace: 'nowrap' }}>
                        <button className="text-button" onClick={() => openEdit(w, 'ward')}>
                          Sửa
                        </button>
                        <button className="text-button" onClick={() => handleDelete(w, 'ward')}>
                          Xóa
                        </button>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan="4" className="empty">
                      {selectedDistrict
                        ? `Chưa có phường/xã trong ${selectedDistrict.ten}.`
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          ...
          >
            <h3>
              {editItem ? 'Sửa' : 'Thêm'} {modalType === 'district' ? 'quận/huyện' : 'phường/xã'}
            </h3>
            <form onSubmit={save}>
              <label>
                Mã *
                <input
                  ...
                  maxLength={20}
                  pattern="[0-9A-Za-z\-]+"
                  title="Chỉ gồm chữ, số và dấu gạch ngang, tối đa 20 ký tự"
                  placeholder="VD: Q01, P-Ba-Dinh"
                />
              </label>
              <label>
                Tên *
              ...
              <label>
                ...
                <textarea
                  ...
                  value={form.boundary}
                  onChange={change}
                  placeholder="Dán GeoJSON MultiPolygon hoặc Feature vào đây…"
                  style={{ minHeight: 100, fontFamily: 'monospace', fontSize: '.82rem' }}
                />
              </label>
              {boundaryError && <div className="field-error">{boundaryError}</div>}
              {previewPolygons.length > 0 && (
                <div>
                  <small style={{ color: '#667085', fontWeight: 600 }}>Preview ranh giới:</small>
                  <MapView polygons={previewPolygons} height="250px" />
                </div>
              )}
