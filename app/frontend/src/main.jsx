...
function Login({ onLogin, notice }) {
  ...
  const goLogin = () => { setResetToken(''); setShowForgot(false); window.history.replaceState(null, '', window.location.pathname); };
  ...
  if (showForgot) return <ForgotPasswordPage onBack={() => setShowForgot(false)} onResetToken={(t) => setResetToken(t)} />;
...
async function downloadDocx(url, filename, notify) { try { const token = localStorage.getItem('qlttxd_token'); const r = await fetch(url, { headers: { 'X-Auth-Token': token || '' } }); if (!r.ok) { let msg = 'Xuất file thất bại'; try { const j = await r.json(); msg = j.error || msg; } catch {} throw new Error(msg); } const blob = await r.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = filename; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(a.href); notify('Đã tải file Word.', 'success'); } catch (e) { notify(errorText(e), 'error'); } }
...
// =========================================================================
function ForgotPasswordPage({ onBack, onResetToken }) {
  ...
  async function submit(e) {
    ...
    try {
      const result = await request('/api/v1/auth/forgot-password', { method: 'POST', body: JSON.stringify({ identifier }) });
      setSent(true);
      if (result.dev_token) setDevToken(result.dev_token);
    }
    catch (err) { setError(errorText(err)); } finally { setBusy(false); }
  }
  if (sent) return <main className="login-shell"><section className="login-card"><div className="brand-mark">QL</div><h1>Kiểm tra email</h1><p>Nếu tài khoản tồn tại, hướng dẫn đặt lại mật khẩu đã được gửi. Token có hiệu lực trong 15 phút.</p>{devToken && <div className="dev-token-box"><p><strong>[DEV]</strong> Token phát hiện — nhấp để đặt lại trực tiếp:</p><button className="forgot-password-link" onClick={() => onResetToken?.(devToken)} style={{ fontSize: '1rem' }}>→ Đặt lại mật khẩu ngay</button></div>}<button className="forgot-password-link" onClick={onBack}>← Quay lại đăng nhập</button></section></main>;
...
// =========================================================================
// T51: Đặt lại mật khẩu — ResetPasswordPage
// =========================================================================
function ResetPasswordPage({ initialToken, onBack }) {
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      await request('/api/v1/auth/reset-password', { method: 'POST', body: JSON.stringify({ token, new_password: password }) });
      setDone(true);
    } catch (err) { setError(errorText(err)); } finally { setBusy(false); }
  }
  if (done) return <main className="login-shell"><section className="login-card"><div className="brand-mark">QL</div><h1>Thành công</h1><p>Mật khẩu đã được đặt lại. Bạn có thể đăng nhập bằng mật khẩu mới.</p><button className="forgot-password-link" onClick={onBack} style={{ fontSize: '1rem' }}>→ Đăng nhập</button></section></main>;
  return <main className="login-shell"><section className="login-card"><div className="brand-mark">QL</div><h1>Đặt lại mật khẩu</h1><p>Nhập mật khẩu mới của bạn.</p>{error && <div className="field-error">{error}</div>}<form onSubmit={submit}>{!initialToken && <label>Token đặt lại<input required value={token} onChange={(e) => setToken(e.target.value)} autoFocus /></label>}<label>Mật khẩu mới (≥8 ký tự, có chữ và số)<input required type="password" minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} autoFocus={!!initialToken} /></label><label>Xác nhận mật khẩu<input required type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} /></label><button disabled={busy}>{busy ? 'Đang đặt lại…' : 'Đặt lại mật khẩu'}</button></form><button className="forgot-password-link" onClick={onBack}>← Quay lại đăng nhập</button></section></main>;
}
