...
-- Lược đồ cơ sở dữ liệu (PostgreSQL 16 + PostGIS)
-- ----------------------------------------------------------------------------
-- Mục đích: Định nghĩa toàn bộ lược đồ dữ liệu cho hệ thống quản lý trật tự
-- xây dựng: phân quyền RBAC, đơn vị hành chính (GIS), danh mục hành vi vi phạm
-- theo Điều 16 Nghị định 16/2022/NĐ-CP, hồ sơ xử lý vi phạm, biên bản, quyết
-- định xử phạt, theo dõi khắc phục hậu quả, tệp đính kèm và nhật ký kiểm toán.
--
-- Cách sử dụng:
--   psql -U <user> -d <db> -f sql/schema.sql
--
-- LƯU Ý: Script này được thiết kế chạy được từ đầu đến cuối trong môi trường
-- cài đặt mới. Các lệnh DROP ... IF EXISTS ... CASCADE ở đầu nhằm đảm bảo tính
-- chạy lại được (idempotent) khi triển khai nhiều lần; cần cẩn trọng khi chạy
-- trên môi trường dữ liệu thật vì sẽ XÓA dữ liệu cũ.
-- ============================================================================
...
-- Kích hoạt extension PostGIS cho dữ liệu không gian (bắt buộc).
...
DROP TABLE IF EXISTS khac_phuc               CASCADE;
DROP TABLE IF EXISTS quyet_dinh              CASCADE;
DROP TABLE IF EXISTS bien_ban                CASCADE;
DROP TABLE IF EXISTS nguoi_vi_pham           CASCADE;
DROP TABLE IF EXISTS ho_so                   CASCADE;
DROP TABLE IF EXISTS bao_cao_vi_pham         CASCADE;
DROP TABLE IF EXISTS muc_phat                CASCADE;
DROP TABLE IF EXISTS hanh_vi_vi_pham         CASCADE;
DROP TABLE IF EXISTS loai_vi_pham            CASCADE;
DROP TABLE IF EXISTS phuong_xa               CASCADE;
DROP TABLE IF EXISTS quan_huyen              CASCADE;
DROP TABLE IF EXISTS role_permissions        CASCADE;
DROP TABLE IF EXISTS user_roles              CASCADE;
DROP TABLE IF EXISTS permissions             CASCADE;
DROP TABLE IF EXISTS roles                   CASCADE;
DROP TABLE IF EXISTS users                   CASCADE;
DROP TYPE   IF EXISTS trang_thai_ho_so       CASCADE;
DROP FUNCTION IF EXISTS next_business_code(TEXT, REGCLASS) CASCADE;
DROP SEQUENCE IF EXISTS code_quyet_dinh_seq CASCADE;
DROP SEQUENCE IF EXISTS code_bien_ban_seq CASCADE;
DROP SEQUENCE IF EXISTS code_ho_so_seq CASCADE;
DROP SEQUENCE IF EXISTS code_bao_cao_seq CASCADE;
...
CREATE SEQUENCE code_bao_cao_seq START WITH 1 INCREMENT BY 1 NO CYCLE;
CREATE SEQUENCE code_ho_so_seq START WITH 1 INCREMENT BY 1 NO CYCLE;
CREATE SEQUENCE code_bien_ban_seq START WITH 1 INCREMENT BY 1 NO CYCLE;
CREATE SEQUENCE code_quyet_dinh_seq START WITH 1 INCREMENT BY 1 NO CYCLE;
...
CREATE INDEX idx_quan_huyen_boundary ON quan_huyen USING GIST (boundary);
CREATE INDEX idx_phuong_xa_boundary   ON phuong_xa  USING GIST (boundary);
CREATE INDEX idx_phuong_xa_huyen      ON phuong_xa  (quan_huyen_id);
...
CREATE TYPE trang_thai_ho_so AS ENUM (
    ...
    'cho_lap_bien_ban',         -- Đủ cơ sở, chờ lập biên bản
    ...
    'dang_khac_phuc',           -- Đang theo dõi khắc phục hậu quả
...
CREATE INDEX idx_tep_dinh_kem_entity ON tep_dinh_kem (entity_type, entity_id);
